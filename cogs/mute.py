...
class MuteCommand(commands.Cog):
    ...
    @commands.slash_command(name="mute", description='Выдаёт таймаут указанному участнику')
    @commands.dynamic_cooldown(functions.give_guild_cooldown, commands.BucketType.user)
    async def slash_mute(self, inter, member: disnake.Member, duration, *, reason):
        try:
    ...
    @commands.slash_command(name="unmute", description='Убирает таймаут указанному игроку')
    @commands.dynamic_cooldown(functions.give_guild_cooldown, commands.BucketType.user)
    async def slash_unmute(self, inter, member: disnake.Member):
        try:
            await functions.chq_moder_role(ctx=inter)
            await member.timeout(duration=0, reason='Требование действующего модератора/администратора сервера.')
            await inter.response.send_message(embed=disnake.Embed(description=f'{member} **был успешно размучен!** :white_check_mark:'))
        except:
            await inter.response.send_message(embed=disnake.Embed(description=f'**Мне не удалось размутить** {member}! **Он имеет роль выше чем я!** :x: \n *Fix error: Сделайте роль бота более приоритетной. Настройки сервера :gear: -> Роли -> Перетащите роль JustB с помощью мышки :mouse2:*'))
