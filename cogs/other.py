...
class OtherCommand(commands.Cog):
    ...
    @commands.slash_command(name="embed", description='Создаёт сообщение типа embed')
    async def slash_embed(self, ctx, color='default', *, reason):
    ...
    @commands.slash_command(name="help", description='Отправляет сообщене о командах бота')
    async def slash_help(self, inter, topic: str = commands.Param(name='topic', choices=['Модераторские команды', 'Голосование', 'Реакции по ролям', 'Прочее'])):
        if topic == 'Модераторские команды':
            await inter.response.send_message(embed=disnake.Embed(description='''**/clear <Количество сообщений>** \n *Удаляет указанное количество сообщений. Существует slash и text версия команды.* \n *Пример использования: /clear 10* 
            ...
            await inter.response.send_message(embed=disnake.Embed(description='''**/add_reactionrole <Эмоджи> <Упоминание роли>** \n \n *Добавляет реакцию на* **reply** *сообщение. Существует только text версия команды.* 
