...
class BanCommand(commands.Cog):
    ...
    @commands.slash_command(description='Перманентно банит пользователя.')
    @commands.dynamic_cooldown(functions.give_guild_cooldown, commands.BucketType.user)
    async def pban(self, ctx, member: disnake.Member, *, reason):
        try:
            await functions.chq_moder_role(ctx=ctx)
            await member.ban(reason=reason)
            await ctx.response.send_message(embed=disnake.Embed(description=f'**{member} заблокирован! :white_check_mark: \n Срок:** Бессрочно \n** Причина:** {reason}'))
        except:
            await ctx.response.send_message(embed=disnake.Embed(description=f'**Мне не удалось заблокировать** {member}! **Он имеет роль выше чем я!** :x: \n *Fix error: Сделайте роль бота более приоритетной. Настройки сервера :gear: -> Роли -> Перетащите роль JustB с помощью мышки :mouse2:*'))
    ...
    @commands.slash_command(description='Банит пользователя на поставленное кол-во времени')
    @commands.dynamic_cooldown(functions.give_guild_cooldown, commands.BucketType.user)
    async def tempban(self, ctx, member: disnake.Member, duration: str = commands.Param(name='time'), *, reason: str = commands.Param(name="reason_for_blocking")):
        try:
            SQL = await DB.cursor()
            await functions.chq_moder_role(ctx=ctx)
            ...
            if ban_time < 600:
                ...
                await member.unban()
                return
            await SQL.execute(f'INSERT INTO ban_members VALUES (?,?,?)', (ctx.guild.id, member.id, int(datetime.datetime.now().timestamp()+ban_time)))
            await DB.commit()
        except:
            await ctx.response.send_message(embed=disnake.Embed(description=f'**Мне не удалось заблокировать** {member}! **Возможные причины:** \n \n  1. Вы неверно указали аргумент duration! Ex. 10s,1m,30d** \n \n2. Пользователь которого вы хотите наказать имеет роль выше чем я!** :x: \n *Fix error: Сделайте роль бота более приоритетной. Настройки сервера :gear: -> Роли -> Перетащите роль JustB с помощью мышки :mouse2:*'))
